...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let group_id = sdk.create_group("Project Team".to_string(), group_members).await?;
    log::info!("Group created with ID: {}", group_id);

    // 4. Send Group Message
    log::info!("Sending group message: 'Hello Team!'");
    sdk.send_to_group(group_id, MessagePayload::Text("Hello Team!".to_string())).await?;
    ...
    sdk.send_to_group(group_id, MessagePayload::Text("Welcome Dave!".to_string())).await?;
