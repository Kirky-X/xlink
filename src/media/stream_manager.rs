...
impl NetworkMonitor {
    ...
    pub fn register_network_change_handler(&mut self, handler: Box<dyn Fn(NetworkType) + Send + Sync>) {
...
impl BitrateController {
    fn new(network_type: NetworkType) -> Self {
        ...
        Self {
            ...
            last_adjustment_time: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(),
        }
    }
    ...
    fn adjust_bitrate(&mut self) {
        ...
        } else if self.rtt_ms < RTT_THRESHOLD_HIGH_MS / 2 && self.packet_loss_rate < PACKET_LOSS_THRESHOLD_HIGH / 2.0 {
            // 网络状况好，提高码率
            self.target_bitrate = (self.current_bitrate as f32 * 1.2) as u32;
        }
        
        // 确保码率在合理范围内
        self.target_bitrate = self.target_bitrate.clamp(VIDEO_BITRATE_MIN, VIDEO_BITRATE_MAX);
        
        // 平滑调整当前码率
        self.current_bitrate = ((self.current_bitrate as f32 * 0.8 + self.target_bitrate as f32 * 0.2) as u32)
...
impl StreamManager {
    ...
    pub async fn handle_chunk(&self, stream_id: Uuid, total_chunks: u32, chunk_index: u32, data: Vec<u8>) -> Result<Option<Vec<u8>>> {
...

        {
            ...
            // 获取或创建会话
            let session = sessions.entry(stream_id).or_insert_with(|| StreamSession::new(StreamType::Data, total_chunks));
            ...
            if session.received_chunks.len() as u32 == session.total_chunks {
                ...
                for i in 0..session.total_chunks {
                    if let Some(chunk) = session.received_chunks.remove(&i) {
                        ...
                        return Err(XPushError::StreamError(format!("Missing chunk {} for stream {}", i, stream_id)));
                    }
                }
    ...
    fn register_network_change_handler(&self) {
        ...
        self.network_monitor.lock().unwrap().register_network_change_handler(
            Box::new(move |new_network| {
                log::info!("Network change detected: {:?}, adjusting stream parameters", new_network);
    ...
    // F8: 发送音频流（专用接口）
    pub async fn send_audio_stream(&self, recipient: DeviceId, audio_data: Vec<u8>, config: Option<AudioConfig>) -> Result<Uuid> {
        ...
        log::info!("Starting audio stream to {} (sample_rate: {}Hz, channels: {}, bitrate: {}bps)", 
                   recipient, audio_config.sample_rate, audio_config.channels, audio_config.bitrate);
        
        // 创建音频特定的流会话
        let stream_id = Uuid::new_v4();
        {
            let mut sessions = self.sessions.lock().unwrap();
            sessions.insert(stream_id, StreamSession {
                total_chunks: 0,
                received_chunks: HashMap::new(),
                last_activity: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(),
                stream_type: StreamType::Audio,
        ...
        self.bitrate_controllers.lock().unwrap().insert(stream_id, bitrate_controller);
        ...
        log::info!("Audio stream {} sent successfully to {}", stream_id, recipient);
        Ok(stream_id)
    }
    ...
    // F8: 发送音频帧
    pub async fn send_audio_frame(&self, recipient: DeviceId, stream_id: Uuid, frame_data: Vec<u8>, timestamp: u64) -> Result<()> {
    ...
    // F8: 发送视频流（专用接口）
    pub async fn send_video_stream(&self, recipient: DeviceId, video_data: Vec<u8>, config: Option<VideoConfig>) -> Result<Uuid> {
        ...
        log::info!("Starting video stream to {} (resolution: {}x{}, fps: {}, bitrate: {}bps)", 
                   recipient, video_config.width, video_config.height, video_config.fps, video_config.bitrate);
        ...
        for (i, chunk) in chunks.iter().enumerate() {
            let chunk_message = Message::new(
                ...
                MessagePayload::StreamChunk {
                    ...
                    data: chunk.clone(),
                    sent_at: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as u64,
                },
            );
        ...
        log::info!("Video stream {} sent successfully to {}", stream_id, recipient);
        Ok(stream_id)
    }
    ...
    // F8: 接收流数据
    pub async fn receive_stream_data(&self, stream_id: Uuid, chunk_index: u32, total_chunks: u32, data: Vec<u8>) -> Result<Vec<u8>> {
        ...
        
        {
            ...
            if session.is_complete() {
                ...
                sessions.remove(&stream_id);
                log::info!("Stream {} completed, received {} chunks", stream_id, total_chunks);
            } else {
                log::debug!("Stream {} progress: {}/{}", stream_id, session.received_chunks.len(), total_chunks);
            }
        }
    ...
    // F8: 处理音频帧
    pub fn process_audio_frame(&self, stream_id: Uuid, frame_data: Vec<u8>, timestamp: u64) -> Result<()> {
        let mut sessions = self.sessions.lock().unwrap();
        if let Some(session) = sessions.get_mut(&stream_id) {
            if session.stream_type == StreamType::Audio {
                ...
                if let Some(ref mut buffer) = session.audio_buffer {
                    ...
                    // 检查缓冲区大小，避免过度累积
                    if buffer.len() > AUDIO_FRAME_SIZE * 100 { // 最多缓存100帧
                        buffer.drain(0..AUDIO_FRAME_SIZE);
                    }
                    
                    log::debug!("Audio frame processed for stream {}, buffer size: {} bytes", stream_id, buffer.len());
                }
    ...
    // F8: 处理视频帧
    pub fn process_video_frame(&self, stream_id: Uuid, frame_data: Vec<u8>, frame_type: FrameType, timestamp: u64) -> Result<()> {
        let mut sessions = self.sessions.lock().unwrap();
        if let Some(session) = sessions.get_mut(&stream_id) {
            if session.stream_type == StreamType::Video {
                ...
                if let Some(ref mut buffer) = session.video_frame_buffer {
                    ...
                    // 检查缓冲区大小，避免过度累积
                    if buffer.len() > 1024 * 1024 { // 最多缓存1MB
                        buffer.clear();
                    }
                    
                    log::debug!("Video frame processed for stream {}, type: {:?}, size: {} bytes", 
                               stream_id, frame_type, frame_data.len());
                }
                ...
                session.priority_queue.sort_by(|a, b| {
                    ...
                    a_priority.cmp(&b_priority).then(a.timestamp.cmp(&b.timestamp))
                });
    ...
    // F8: 自适应码率调整
    pub fn adjust_stream_bitrate(&self, stream_id: Uuid, rtt_ms: u32, packet_loss_rate: f32) -> Result<u32> {
        let mut controllers = self.bitrate_controllers.lock().unwrap();
        if let Some(controller) = controllers.get_mut(&stream_id) {
            ...
            log::info!("Adjusted bitrate for stream {} to {} bps (RTT: {}ms, Loss: {:.2}%)", 
                       stream_id, new_bitrate, rtt_ms, packet_loss_rate * 100.0);
            Ok(new_bitrate)
        } else {
    ...
    pub fn get_traffic_statistics(&self) -> TrafficStatistics {
        ...
        for (_stream_id, session) in sessions.iter() {
            if let Some(stats) = &session.network_stats {
                ...
                let app_traffic = app_breakdown.entry(app_name.to_string()).or_insert(AppTraffic {
                    bytes_sent: 0,
                    bytes_received: 0,
                    packets_sent: 0,
                    packets_received: 0,
    ...
    pub fn check_traffic_alerts(&self, stats: &TrafficStatistics) {
        ...
        if total_bytes > TRAFFIC_ALERT_THRESHOLD_BYTES {
            log::warn!("Traffic Alert: Total data usage ({:.2} MB) exceeds threshold ({:.2} MB)", 
