...
async fn test_100mb_file_transmission() {
    ...
    let group_id = sender_sdk
        .create_group(
            "File Transfer Group".to_string(),
            vec![sender_id, receiver_id],
        )
        .await
        .unwrap();

    // UAT requirement: Test 3 files for success rate ≥95% (reduced from 10 for faster testing)
    let num_files = 3;
    let mut successful_transmissions = 0;
    let mut transmission_times = Vec::new();

    println!(
        "Testing {} 100MB files for UAT success rate requirement...",
        num_files
    );
    ...
    for i in 0..num_files {
        ...
        // Optimized: create pattern once and repeat
        ...
        // Send the large file as binary payload with timeout
        let send_timeout = Duration::from_secs(30);
        let result = timeout(
            send_timeout,
            sender_sdk.send_to_group(group_id, MessagePayload::Binary(test_data)),
        )
        .await;
        ...
        let result = match result {
            ...
            Err(_) => {
                println!(
                    "❌ File {} transmission timed out after {:?}",
                    i + 1,
                    send_timeout
                );
                continue;
        ...
        if result.is_ok() {
            ...
            println!(
                "✅ File {} transmitted successfully in: {:?}",
                i + 1,
                transmission_time
            );
        } else {
    ...
    if !transmission_times.is_empty() {
        let avg_time =
            transmission_times.iter().sum::<Duration>() / transmission_times.len() as u32;
    ...
    assert!(
        success_rate >= 95.0,
        "UAT requirement failed: Success rate ({:.1}%) is below 95% requirement",
        success_rate
    );
    ...
    for &time in &transmission_times {
        assert!(
            ...
            "UAT requirement failed: Transmission time ({:?}) exceeds 10 minutes limit",
            time
        );
    }
...
async fn test_100mb_file_transmission_with_resume() {
    ...
    let group_id = sender_sdk
        .create_group(
            "File Transfer Resume Group".to_string(),
            vec![sender_id, receiver_id],
        )
        .await
        .unwrap();
    ...
    println!(
        "Sending 100MB file in {} chunks to test resume capability...",
        chunks.len()
    );
    ...
    assert!(
        total_transmission_time <= Duration::from_secs(600),
        "UAT requirement failed: Total transmission time ({:?}) exceeds 10 minutes limit",
    ...
    assert!(
        ...
        "UAT requirement failed: Not all chunks were transmitted successfully ({} out of {})",
        sent_chunks,
        chunks.len()
    );
    ...
    println!(
        "   - Total transmission time: {:?} (≤10 minutes)",
        total_transmission_time
    );
...
async fn test_100mb_file_transmission_with_interruption() {
    ...
    let result = sender_sdk
        .send_to_group(group_id, MessagePayload::Binary(test_data))
        .await;

    let transmission_time = start_time.elapsed();
    println!(
        "100MB file transmission with interruption took: {:?}",
        transmission_time
    );
    ...
    assert!(
        result.is_ok(),
        "100MB file transmission with interruption failed: {:?}",
        result.err()
    );
