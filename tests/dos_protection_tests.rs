...
use crate::common::{TestSdkBuilder, test_device_id, NetworkSimulator};
...
async fn simulate_dos_attack(
    ...
    println!("Starting DoS attack simulation: {} requests/second for {:?}", requests_per_second, attack_duration);
    
    while start_time.elapsed() < attack_duration {
        let mut handles: Vec<JoinHandle<Result<bool, Box<dyn std::error::Error + Send + Sync>>>> = Vec::new();
        ...
        for i in 0..requests_per_second {
            ...
            let handle = tokio::spawn(async move {
                // 使用真实的SDK调用来测试速率限制
                let result = sdk_clone.send(target_device, MessagePayload::Text(message)).await;
                match result {
                    Ok(_) => Ok(true),
    ...
    println!("DoS attack completed: {} successful, {} blocked", successful_requests, blocked_requests);
...
async fn test_rate_limiting(
    ...
    println!("Testing rate limiting with {} burst requests...", burst_requests);
    ...
    for i in 0..burst_requests {
        ...
        let handle = tokio::spawn(async move {
            sdk_clone.send(target_device, MessagePayload::Text(message)).await
        });
    ...
    for (handle, i) in handles {
        ...
        if i % 50 == 49 {
            ...
            println!("Progress: {} requests sent in {:?} ({:.1} req/sec)", 
                    i + 1, elapsed, (i + 1) as f64 / elapsed.as_secs_f64());
        }
    }
    
    let total_time = start_time.elapsed();
    println!("Rate limiting test completed: {} successful, {} rate limited in {:?} ({:.1} req/sec)", 
            successful_requests, rate_limited_requests, total_time, 
...
async fn test_dos_protection_rate_limiting() {
    ...
    println!("   - System blocked {} out of {} attack requests", rate_limited, total_requests);
...
async fn test_dos_protection_sustained_attack() {
    ...
    // 模拟持续攻击
    let (successful, blocked) = simulate_dos_attack(&sdk, target_device, attack_duration, attack_rate).await.unwrap();
...
async fn test_dos_protection_recovery() {
    ...
    let (normal_success, normal_limited) = test_rate_limiting(&sdk, target_device, 50).await.unwrap();
    assert_eq!(normal_success, 50, "Normal requests should all succeed");
    assert_eq!(normal_limited, 0, "Normal requests should not be rate limited");
    
    // 第二阶段：模拟攻击（大量请求）
    println!("Phase 2: Simulating attack...");
    let (_attack_success, attack_limited) = test_rate_limiting(&sdk, target_device, 150).await.unwrap();
    ...
    let (recovery_success, recovery_limited) = test_rate_limiting(&sdk, target_device, 30).await.unwrap();
    assert_eq!(recovery_success, 30, "Recovery requests should succeed");
    assert_eq!(recovery_limited, 0, "Recovery requests should not be rate limited");
...
async fn test_dos_protection_edge_cases() {
    ...
    let (boundary_success, boundary_limited) = test_rate_limiting(&sdk, target_device, 100).await.unwrap();
    println!("  Results: {} successful, {} rate limited", boundary_success, boundary_limited);
    ...
    println!("  Results: {} successful, {} rate limited", over_success, over_limited);
    ...
    let result = sdk.send(target_device, MessagePayload::Text("Single request".to_string())).await;
...
async fn test_uat_dos_protection_comprehensive() {
    ...
    let normal_result = sdk.send(target_device, MessagePayload::Text("Normal user request".to_string())).await;
    assert!(normal_result.is_ok(), "UAT requirement failed: Normal requests should always be possible");
    ...
    let attack_rate = 300;
    let (successful_attacks, blocked_attacks) = simulate_dos_attack(&sdk, target_device, attack_duration, attack_rate).await.unwrap();
    ...
    println!("   Attack results: {} successful, {} blocked", successful_attacks, blocked_attacks);
    println!("   Blocked ratio: {:.1}%", attack_blocked_ratio);
    
    // UAT requirement: 系统应阻止大部分攻击请求
    assert!(blocked_attacks > 0, "UAT requirement failed: System should block attack requests");
    assert!(attack_blocked_ratio >= 15.0, "UAT requirement failed: Attack blocked ratio ({:.1}%) too low", attack_blocked_ratio);
    
    // Test 3: 验证攻击后系统恢复能力
    println!("\n3. Testing system recovery after attack...");
    sleep(Duration::from_secs(2)).await; // 等待恢复时间
    
    let recovery_result = sdk.send(target_device, MessagePayload::Text("Recovery test request".to_string())).await;
    assert!(recovery_result.is_ok(), "UAT requirement failed: System should recover after attack");
    ...
    for i in 0..availability_test_count {
        let result = sdk.send(target_device, MessagePayload::Text(format!("Availability test {}", i))).await;
        if result.is_ok() {
            successful_normal_requests += 1;
        }
    }
    
    let availability_ratio = successful_normal_requests as f64 / availability_test_count as f64 * 100.0;
    println!("   Service availability: {:.1}% ({}/{} requests)", availability_ratio, successful_normal_requests, availability_test_count);
    
    // UAT requirement: 服务可用性应保持在较高水平
    assert!(availability_ratio >= 90.0, "UAT requirement failed: Service availability ({:.1}%) too low", availability_ratio);
    
    // Summary
    println!("\n=== UAT DoS Protection Summary ===");
    println!("✅ Normal request availability: Always available");
    println!("✅ Attack protection: {:.1}% of {} attack requests blocked", attack_blocked_ratio, total_attack_requests);
    println!("✅ System recovery: Successful after attack");
    println!("✅ Service availability: {:.1}% during testing", availability_ratio);
