...
use crate::common::{test_device_id, test_device_capabilities, TestSdkBuilder, NetworkSimulator};
...
async fn test_create_and_manage_group() {
    ...
    let creator_pk = x25519_dalek::PublicKey::from(&x25519_dalek::StaticSecret::random_from_rng(rand::rngs::OsRng));
    group_manager.register_device_key(creator_id, creator_pk).unwrap();
...
async fn test_treekem_group_security() {
    ...
    let group_id = sdk.create_group("Secure Group".to_string(), all_members).await.unwrap();
...
async fn test_multi_device_group_broadcast() {
    ...
    let group_id = sdk.create_group("Broadcast Group".to_string(), device_ids).await.unwrap();
...
async fn test_large_group_performance() {
    ...
    let group_id = sdk.create_group("Large Group".to_string(), device_ids).await.unwrap();
    ...
    assert!(result.is_ok());
    println!("500-person group: creation={:?}, broadcast={:?}", creation_time, broadcast_time);
}
