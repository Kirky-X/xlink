...
async fn create_memory_sdk(device_id: DeviceId) -> Result<UnifiedPushSDK, Box<dyn std::error::Error>> {
    ...
    impl MessageHandler for NoOpHandler {
        async fn handle_message(&self, _message: xpush::core::types::Message) -> xpush::core::error::Result<()> {
            Ok(())
        }
    }
        ...
        match get_memory_usage().await {
            Ok(memory_kb) => {
                ...
                if samples.len() > 1 {
                    ...
                    println!("Memory usage: {} KB (increase: {} KB)", current_memory, memory_increase);
...
async fn test_long_running_7_days_no_memory_leak() {
    ...
    // 由于实际运行7天测试不现实，这里运行一个缩短版本（7分钟）
    // 用于验证内存监控机制和基本功能
    let test_duration = Duration::from_secs(7 * 60); // 7分钟代替7天
    ...
    if samples.len() >= 2 {
        ...
        println!("  Memory increase: {} KB ({:.2} MB)", memory_increase, memory_increase as f64 / 1024.0);
        
        // 检查内存增长趋势
        // 在7分钟的测试中，内存增长应该很小（小于10MB）
        assert!(
            memory_increase < 10 * 1024,
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            while running_clone.load(Ordering::Relaxed) {
                ...
                // 发送消息
                let _ = sdk_clone.send(device_id_clone, MessagePayload::Text(message)).await;
...
async fn test_resource_cleanup() {
    ...
    for _i in 0..100 {  // 创建100个SDK实例进行测试
        let device_id = test_device_id();
