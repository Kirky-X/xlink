...
fn test_rate_limiter_dashmap_leak() {
    ...
    println!("Memory after inserting 10k entries: {} KB", after_insert_memory);
    println!("Memory increase: {} KB", after_insert_memory - initial_memory);
    ...
    println!("Memory retained after clear: {} KB", after_clear_memory - initial_memory);
    ...
    println!("\nMemory after inserting 10k entries (second test): {} KB", after_insert2_memory);
    
    // Remove entries one by one instead of using clear()
    let keys: Vec<_> = rate_limiter2.iter().map(|entry| entry.key().clone()).collect();
    for key in keys {
    ...
    println!("Memory retained after remove: {} KB", after_remove_memory - initial_memory);
    ...
    println!("Total memory leaked: {} KB", final_memory.saturating_sub(initial_memory));
    ...
    let difference = (clear_retained as i64 - remove_retained as i64).abs() as u64;
    assert!(difference <= 128, "Both approaches should retain similar memory, but difference is {} KB", difference);
    
    // The total retained memory should be reasonable for DashMap structure (< 256 KB)
    assert!(clear_retained <= 256, "Clear approach retained too much memory: {} KB", clear_retained);
    assert!(remove_retained <= 256, "Remove approach retained too much memory: {} KB", remove_retained);
}