...
use crate::common::{TestSdkBuilder, NetworkSimulator, establish_device_sessions, test_device_id};
...
async fn test_office_file_sharing_scenario() {
    ...
    let device_ids = vec![alice_sdk.device_id(), bob_sdk.device_id(), charlie_sdk.device_id()];
    
    // Establish sessions
    establish_device_sessions(&[&alice_sdk, &bob_sdk, &charlie_sdk]).await.unwrap();
    
    let office_group = alice_sdk.create_group("Office Team".to_string(), device_ids).await.unwrap();
    
    let presentation_data = vec![0u8; 1024 * 1024]; // 1MB for test
    let result = alice_sdk.send_to_group(office_group, MessagePayload::Binary(presentation_data)).await;
...
async fn test_emergency_communication_scenario() {
    ...
    establish_device_sessions(&[&responder_sdk, &coordinator_sdk]).await.unwrap();
    
    let result = responder_sdk.send(coordinator, MessagePayload::Text("EMERGENCY: Need backup!".to_string())).await;
    assert!(result.is_ok(), "Emergency message should be delivered even in poor network");
}
...
async fn test_storage_cleanup() {
    ...
    // Save some messages
    for i in 0..20 {
        let msg = xpush::core::types::Message::new(sender, recipient, MessagePayload::Text(format!("Msg {}", i)));
...
async fn test_system_recovery_after_restart() {
    ...
    // 1. First run: Create a group and save state
    {
        ...
        sdk.create_group("Persistent Group".to_string(), vec![caps.device_id]).await.unwrap();
    } // SDK dropped here
