    ...
    async fn test_memory_storage_cleanup() {
        ...
        // Create and fill MemoryStorage similar to SDK usage
        {
            ...
            for i in 0..1000 {
                ...
                storage.save_audit_log(format!("Audit log {}", i)).await.unwrap();
            }
            
            let after_fill = get_memory_usage().await.unwrap();
            println!("After filling storage: {} KB (increase: {} KB)", after_fill, after_fill - initial_memory);
            ...
            let cleanup_change = if after_cleanup >= after_fill { 
                after_cleanup - after_fill 
            } else { 
                0 
            };
            println!("After cleanup: {} KB (change: {} KB)", after_cleanup, cleanup_change);
        ...
        let net_change = if final_memory >= initial_memory { 
            final_memory - initial_memory 
        } else { 
            0 
        };
        println!("Final memory: {} KB (net change: {} KB)", final_memory, net_change);
