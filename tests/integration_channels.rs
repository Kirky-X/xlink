...
async fn test_server_failover_on_primary_failure() {
    ...
    assert_eq!(remote_channel1.current_server_url().await, "https://ntfy-primary.sh");
    
    remote_channel1.switch_to_next_server().await;
    assert_eq!(remote_channel1.current_server_url().await, "https://ntfy-backup1.sh");
    
    let result = sdk1.send(device2_id, MessagePayload::Text("Failover test".to_string())).await;
    assert!(result.is_ok());
}
...
async fn test_channel_switching_wifi_to_ble() {
    ...
    let result = sdk.send(device2, MessagePayload::Text("Switching test".to_string())).await;
    assert!(result.is_ok());
}
...
async fn test_channel_switching_performance() {
    ...
    let result = sdk.send(device2, MessagePayload::Text("Performance test".to_string())).await;
